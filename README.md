...
### 🌐 Bunny Database & Storage Integration
...
> [!NOTE]
> **Bunny DB Setup**: Requires `@prisma/adapter-libsql@6.19.2` (must match your Prisma engine version). The schema syncer uses `PRAGMA table_info()` to detect and apply only missing columns via `ALTER TABLE ADD COLUMN`, making schema evolution safe.
...
- Falls back to local storage if Bunny Storage is unavailable
