...
| SEO | Site-wide and per-post metadata, canonical URLs, Open Graph/Twitter cards, generated share images, WebSite/BlogPosting/Breadcrumb/ProfilePage+Person JSON-LD, dynamic `sitemap.xml` (including `/profile` when enabled), `robots.txt`, public RSS 2.0 at `/feed.xml` (alias `/rss.xml`), search verification, indexing controls, and optional `llms.txt`. Site SEO and per-post overrides are agent-manageable via the REST API. |
