...
## 2026-05-15 - Conditional Hooks CI Failure
**Learning:** React 'Rules of Hooks' violations (like hooks called conditionally or after early returns) will cause the Next.js production build (`next build`) and the associated CI pipeline to fail, even if the code runs fine in development.
**Action:** Always define `useMemo` and other hooks at the absolute top level of functional components, strictly before any conditional logic or early returns (e.g., `if (loading) return`).
