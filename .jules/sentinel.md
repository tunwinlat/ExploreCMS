...
## 2026-04-05 - Rate Limit IP Spoofing
**Vulnerability:** Several Next.js Server Actions were manually parsing HTTP headers (`x-forwarded-for`, `x-real-ip`, `cf-connecting-ip`) to determine the client IP for rate limiting, completely bypassing the `isTrustedProxy` checks implemented in the core rate limit library. This allowed any user to trivially spoof their IP address and bypass authentication rate limits.
**Learning:** Security logic (like secure proxy-aware IP resolution) was duplicated rather than imported, leading to critical omissions when developers copied code snippets between files. Server Actions often need the same rigorous header validation as API routes.
**Prevention:** Never manually parse proxy headers directly in route handlers or server actions. Always use a centralized, tested utility (like `getClientIPFromHeaders`) that strictly enforces trusted proxy validations.
