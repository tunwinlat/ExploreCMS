...
describe('updateNavigationConfig', () => {
  ...
  it('should throw an error if no session exists', async () => {
    (verifySession as import('vitest').Mock).mockResolvedValueOnce(null)
    await expect(updateNavigationConfig('{}')).rejects.toThrow('Unauthorized')
    expect(prisma.siteSettings.upsert).not.toHaveBeenCalled()
  })

  it('should throw an error if role is not OWNER', async () => {
    (verifySession as import('vitest').Mock).mockResolvedValueOnce({ role: 'ADMIN' })
    await expect(updateNavigationConfig('{}')).rejects.toThrow('Unauthorized')
    expect(prisma.siteSettings.upsert).not.toHaveBeenCalled()
  })

  it('should update navigation config if session is OWNER', async () => {
    (verifySession as import('vitest').Mock).mockResolvedValueOnce({ role: 'OWNER' })
    ;(prisma.siteSettings.upsert as import('vitest').Mock).mockResolvedValueOnce({})
  ...
  it('should throw an error if database upsert fails', async () => {
    (verifySession as import('vitest').Mock).mockResolvedValueOnce({ role: 'OWNER' })
    ;(prisma.siteSettings.upsert as import('vitest').Mock).mockRejectedValueOnce(new Error('DB Error'))
