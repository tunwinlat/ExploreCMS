    ...
    for (let i = 0; i < repoFullNames.length; i += chunkSize) {
      ...
      const chunkPromises = chunk.map(async (fullName) => {
        try {
          ...
          if (existing) {
            slug = `${slug}-${Date.now()}-${Math.floor(Math.random() * 10000)}`
          }
      ...
      for (const result of chunkResults) {
        if (result.status === 'fulfilled') {
          ...
          // This shouldn't normally happen since we catch errors inside the map,
          // but it's safe to handle just in case.
          results.push({ success: false, name: 'Unknown', error: String(result.reason) })
        }
      }
    }
  ...
  try {
    ...
    for (let i = 0; i < projects.length; i += chunkSize) {
      ...
      const chunkPromises = chunk.map(async (project) => {
        try {
          if (!project.githubRepoFullName) {
             return null // silently skip to match original behavior
          }
